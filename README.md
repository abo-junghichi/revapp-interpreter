...
実行速度を上げてRAMの消費量を減らした、もはや最低限とは言えない処理系です。
現在、バイトコードにインタプリタを付加してコンパイルした、
実行ファイルの生成のみを実装しています。
また、最適化機能を追加しているので、
空文字列への束縛「=」を用いた表記でスペースリークが生じず、
コメント文として安全に使えます。
...
なので、一見するとこれをコメント文として使い回せそうです。
しかし最低限の実装である当処理系では、
ガベージコレクタがこのことを認識するのが大幅に遅れ、
スペースリークが生じる場合があります。
なので、シェルスクリプトのshebang行も兼ねて、
文字'#'で始まる行をコメント文として当処理系は扱います。
